...
def remove():
    ...
    headers = {'xi-api-key': st.session_state["elevenlabs_key"],}
...
def run():
    ...
    try:
        ...
        with st.spinner("Transcribing..."):
            ...
            try:
                with concurrent.futures.ThreadPoolExecutor(max_workers=num_threads) as executor:
                    args_list = [(chunk, st.session_state.openai_key) for chunk in st.session_state.chunks]
        ...
        with st.spinner("Translating..."):
            ...
            try:
                with concurrent.futures.ThreadPoolExecutor(max_workers=num_threads) as executor:
                    args_list = [(chunk, st.session_state.openai_key, st.session_state.source_lang, st.session_state.target_lang) for chunk in st.session_state.chunks]
        ...
        with st.spinner("Generating Audio..."):
            start = time.time()
            st.session_state.chunks, st.session_state.voice = db.audio_synthesis(st.session_state.chunks, st.session_state.elevenlabs_key)
            end = time.time()
...
def generate():
    with st.spinner("Generating Audio..."):
        start = time.time()
        st.session_state.current_chunk = db.audio_synthesis(chunks=[st.session_state.current_chunk], key=st.session_state.elevenlabs_key,
...
def main():
    ...
    if not (st.session_state.get("openai_key") and st.session_state.get("elevenlabs_key")):
        with st.form(key="info_form"):
            st.text_input("OpenAI Api Key", key='openai_key')
            st.text_input("11Labs Api Key", key='elevenlabs_key')
            st.form_submit_button(label='Submit')

    if 'video_path' not in st.session_state and st.session_state.get("openai_key") and st.session_state.get("elevenlabs_key"):
        with st.container():
